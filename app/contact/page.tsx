...
export default function ContactPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      ...
      <p className="text-neutral-700 mb-6">Get a quick quote or book a site visit.</p>
      {ok ? (
        <div className="p-4 rounded-md bg-green-50 border text-green-700">Thanks! We will get back within 1 working day.</div>
      ) : (
        <form action={onSubmit} className="grid gap-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <input name="address" placeholder="Site address" className="border rounded-md px-3 py-2"/>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            <select name="propertyType" className="border rounded-md px-3 py-2">
              <option>Home</option><option>Apartment</option><option>Commercial</option><option>Industrial</option>
            </select>
            <input name="monthlyBill" type="number" placeholder="Monthly KSEB bill (₹)" className="border rounded-md px-3 py-2"/>
      ...
      <div id="site-visit" className="mt-8 rounded-xl border p-4 bg-neutral-50">
        <h2 className="font-semibold mb-2">Our Location</h2>
        <div className="aspect-video bg-neutral-200 rounded-md flex items-center justify-center text-neutral-600">Google Map Placeholder</div>
      </div>
    </div>
  );
}
