...
export function Estimator() {
  ...
  return (
    <div className="rounded-xl border p-6 bg-white shadow-sm">
      ...
      <input type="number" className="w-full border rounded-md px-3 py-2 mb-3"
        value={bill} onChange={(e)=>setBill(e.target.value)} min={0} />
      <button className="px-4 py-2 rounded-md bg-primary text-white text-sm"
        onClick={()=>setRes(estimateSystemFromBill(Number(bill)||0))}>Estimate Savings</button>
      {res && (
      ...
      <p className="mt-3 text-xs text-neutral-500">* Rough estimates. Site results vary with usage, orientation & shading.</p>
    </div>
  );
}
