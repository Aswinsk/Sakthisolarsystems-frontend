    ...
    <header className="sticky top-0 z-50 bg-white shadow">
      <div className="mx-auto max-w-7xl px-4 h-[var(--header-height)] flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3">
          <Image src="/logo.svg" width={160} height={36} alt="Sakthi Solar Systems logo" priority />
        ...
        <div className="flex items-center gap-2">
          ...
          <a href="tel:+91XXXXXXXXXX" className="px-3 py-2 rounded-md bg-primary text-white text-sm">
            {t(locale, "common.phone")}
          </a>
          <div className="ml-2 border rounded-md overflow-hidden">
            <button onClick={() => switchLocale("en")} className={`px-2 py-2 text-xs ${locale === "en" ? "bg-neutral-100" : ""}`}>EN</button>
            <button onClick={() => switchLocale("ml")} className={`px-2 py-2 text-xs ${locale === "ml" ? "bg-neutral-100" : ""}`}>ML</button>
          </div>
        </div>
      </div>
